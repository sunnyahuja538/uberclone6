        ...
        <div className='fixed z-10 translate-y-full w-full flex flex-col gap-2 bg-white bottom-0 px-3 py-6' ref={props.vehicleRef}>
              <h5 className='ml-[85%] absolute' onClick={()=>{
                ...
                <div className='flex p-3 border-2 active:border-black rounded-lg w-full items-center justify-between' onClick={()=>{
                  ...
                  <img className='h-13' src="https://www.uber-assets.com/image/upload/f_auto,q_auto:eco,c_fill,h_368,w_552/v1548646935/assets/64/93c255-87c8-4e2e-9429-cf709bf1b838/original/3.png"/>
                  ...
                  <h2 className=' font-semibold'>Rs.{props.fare.car}</h2>
                </div>
                <div className='flex p-3 border-2 active:border-black  rounded-lg w-full items-center justify-between' onClick={()=>{
                  props.setConfirm(true);
                  ...
                  <h2 className=' font-semibold'>Rs.{props.fare.bike}</h2>
                </div>
                <div className='flex p-3 border-2 active:border-black  rounded-lg w-full items-center justify-between' onClick={()=>{
                  props.setConfirm(true);
                  props.setvehicleType('auto');
                }}>
                  <img className='h-13' src="https://www.uber-assets.com/image/upload/f_auto,q_auto:eco,c_fill,h_368,w_552/v1648431773/assets/1d/db8c56-0204-4ce4-81ce-56a11a07fe98/original/Uber_Auto_558x372_pixels_Desktop.png"/>
                  <div className=' ml-2 w-1/2'>
                    <h4 className='font-semibold text-sm'>
                  ...
                  <h2 className=' font-semibold'>Rs.{props.fare.auto}</h2>
                </div>
                </div>
    </div>
  )
...
// JSX: Allows direct mixing of strings and expressions (e.g., Rs{fare}) without concatenation.

// JavaScript: Requires explicit concatenation using + or template literals (e.g., "Rs" + fare).

// C++: Needs explicit stream insertion (<<) or string conversion ("Rs" + to_string(fare)) — no automatic merging.